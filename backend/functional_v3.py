...
async def generate_dialogue(session_id: str, assignment: str, unit: str, student_text: str,
                            ...
                            structural_scaffold: str = "", selection_phase: str = "") -> str:
    ...
    if _reduction_mode:
        ...
        _ops_block = ""
        _reduction_common = (
            "THE INSTRUCTIONAL DECISION IS ALREADY MADE (internal — never named to the learner): this turn "
            "helps the learner decide what THIS paragraph should carry; it is NOT building, elaborating, "
            "unfolding, or developing any structure. The learner already has enough conceptual material; "
            "do NOT coach them to add, build, unfold, or elaborate anything, and do NOT introduce a "
            "canonical structure name to 'work on'. LEARNER LANGUAGE: describe the LEARNER'S work, never "
            "Compass's internal operation — do NOT say 'structural selection', 'reduction', 'decomposition', "
            "or 'operation'; say things like 'deciding what this paragraph should carry' or 'which ideas "
            "belong together here'. WORKING-MEMORY RULE: never make the learner hold several ideas in mind "
            "AND perform another operation at once — EXTERNALIZE the relevant structure on screen, and ask "
            "for only ONE cognitive operation this turn. CHUNKING RULE: when you externalize several items, "
            "ORGANIZE them into a few meaningful conceptual GROUPS that are already implicit in the "
            "learner's own writing (e.g. Problem / Causes / Solution / Example / Supporting mechanism / "
            "Conclusion — but derive the groups and their plain-language labels from THEIR draft; use no "
            "fixed template and NEVER invent organization the draft does not support). Externalization "
            "reduces memory load; grouping reduces organizational load and teaches how experts organize "
            "material. Never rewrite the paragraph for them, and NEVER suggest the assignment should be an "
            "essay.\n"
        )
        if (selection_phase or "").strip().lower() == "rewrite":
            # PHASE 2 — the learner has just chosen a subset (see their message). Confirm + one rewrite op.
            _focus_block = (
                _reduction_common +
                "PHASE 2 of 2 — THE LEARNER HAS JUST CHOSEN which lines to keep (their choice is in THE "
                "LEARNER'S MESSAGE THIS TURN, e.g. 'B, C, E'). Do EXACTLY this, in order:\n"
                "1. RE-SHOW THEIR SELECTION so they need not remember it: using the lettered list below, "
                "map their choice back to the actual arguments and repeat the chosen ones with their full "
                "wording (e.g. 'Good — you chose: B. … C. … E. …'). Confirm warmly that these form a "
                "manageable single line of development. If their message is ambiguous, briefly restate "
                "what you understood and proceed with the closest reasonable reading.\n"
                "2. GIVE ONE OPERATION ONLY: tell them to rewrite the paragraph using ONLY those chosen "
                "ideas, adding nothing new yet; use the first chosen point to state the main explanation "
                "and the others to develop it. Then STOP and RESUBMIT.\n"
                "Do NOT re-list all the options, do NOT ask them to choose again, and do NOT ask for "
                "sentence improvement, grammar, transitions, new evidence, examples, or a counterargument.\n"
                + (structural_scaffold or "")
            )
        else:
            # PHASE 1 — externalize the structure and ask ONLY for a selection (no rewrite yet).
            _focus_block = (
                _reduction_common +
                "PHASE 1 of 2 — this turn ends after the learner CHOOSES; do NOT ask for a rewrite yet. "
                "A novice may understand 'this paragraph does too much' yet not know what to do, so make "
                "the hidden organization of THEIR OWN paragraph visible, then ask for ONE choice. Do these "
                "in order, in warm plain language:\n"
                "1. DIAGNOSE PLAINLY (do not soften into a vague 'which part seems most important?'): say "
                "directly that the paragraph is trying to do too much at once, that they have enough "
                "ideas, and that the work now is choosing which ideas THIS ONE paragraph should carry.\n"
                "2. RE-ANCHOR TO THE THESIS: restate, in one sentence, the central idea already organizing "
                "their paragraph (Compass's reading of THEIR thesis, not a new one). If confidence is low, "
                "phrase it provisionally ('I think your central idea is…').\n"
                "3. EXTERNALIZE THE LOCAL ARGUMENTS AS GROUPED CHUNKS: do NOT dump a flat list. Infer the "
                "few higher-level conceptual groups already implicit in their paragraph and display each "
                "group under a short plain-language heading, with its member arguments (each still "
                "letter-labeled) indented beneath it — for example a 'Problem' group, a 'Solution' group, "
                "an 'Example' group, a 'Supporting mechanism' group, a 'Conclusion' group, drawn from THEIR "
                "writing. Word the arguments from their actual draft; invent nothing that is not there.\n"
                "4. SHOW THE STRUCTURAL PROBLEM: explain these groups are all related but are not all doing "
                "the same local job for THIS paragraph — several could anchor SEPARATE paragraphs — and "
                "teach the distinction between 'related to the topic' and 'belongs in THIS paragraph'.\n"
                "5. ASK FOR ONE CHOICE ONLY, PREFERABLY AT THE GROUP LEVEL FIRST: guide them to decide "
                "which GROUPS this paragraph most needs (e.g. 'this paragraph probably needs the idea in "
                "the Problem group and one or two from the Solution group'), then to name the two or three "
                "specific lettered ideas within those groups. If they seem unsure, be more directive — "
                "recommend a specific small subset that forms one clear line of development — while making "
                "clear the final choice is theirs. END THE TURN HERE: do NOT ask them to rewrite, cut, or "
                "do sentence work yet.\n"
                + (structural_scaffold or "")
            )
    else:
        _focus_block = (
...
_FUNCTION_SEL_SYS = (
    ...
    "\n"
    "OUTPUT DISCIPLINE (CRITICAL — protects interactive latency; does NOT reduce your reasoning). "
    "Reason THOROUGHLY and completely INTERNALLY, but the JSON you EMIT must be MAXIMALLY COMPACT:\n"
    "(1) Every free-text/narrative string value <= 20 words: state the JUDGMENT, not its justification, "
    "and never restate/paraphrase the schema's field descriptions.\n"
    "(2) OMIT every `evidence` array entirely (do not emit the key). Emit `confidence` only as a bare "
    "h|m|l where the schema asks for it; never expand it.\n"
    "(3) PRESERVE IN FULL and keep accurate every LEARNER-CRITICAL structured field, because downstream "
    "deterministic logic depends on them: structural_load_analysis (central_communicative_movement, "
    "current_structural_work[], secondary_trajectories[], competing_structural_work[], "
    "redundant_structural_work[], structural_load_status, structural_pruning_needed), "
    "learner_relative_sufficiency (value, learner_accessible_target, accessible_target_achieved, "
    "episode_target_status, episode_target_revision_reason), task_relative_adequacy.value, "
    "timely_success_status.value, communicative_capacity, learner_orientation (all sub-fields, terse), "
    "sentence_craft_readiness.value, completion_readiness.value, completion_message (when ready). "
    "Keep every LIST complete — do not drop items to save space.\n"
    "(4) For all OTHER analytic fields, emit a terse value (<= 20 words) or omit the field if it would "
    "be empty. Goal: the SMALLEST JSON that fully preserves the learner-critical fields above.\n"
    "\n"
    + _FUNCTION_DEFINITIONS + "\n"
    "\n"
...
async def _select_functions(session_id: str, assignment: str, unit: str, student_text: str,
    ...
    chat = LlmChat(api_key=_KEY, session_id=f"fn-sel-{session_id}",
                   system_message=_FUNCTION_SEL_SYS).with_model(*SEL_MODEL).with_params(max_tokens=8000)
    raw = await chat.send_message(UserMessage(text=prompt))
    try:
...
async def run(session: Dict[str, Any], learner_content: str, kind: str) -> Dict[str, Any]:
    ...
    if _sc_active_turn:
        ...
        if _operation in ("structural_selection", "condense_and_integrate"):
            # 4.9.4 — split into two small steps: PHASE 1 externalize + ask to SELECT; PHASE 2 (after the
            # learner replies with a selection message) confirm the subset + give ONE rewrite operation.
            if state.awaiting_structural_selection and (learner_message or "").strip():
                _sel_phase = "rewrite"
                state.awaiting_structural_selection = False
            else:
                _sel_phase = "select"
                state.awaiting_structural_selection = True
...
async def sentence_craft_cognition(session_id: str, prompt: str, draft: str,
    ...
    p = (
        ...
        "TASK: (1) Give a COMPACT analysis of EVERY sentence by index (do not omit any). Name CONCRETE "
        "phenomena from the RUBRIC PATTERN VOCABULARY (e.g. 'to be' main verb, run-on, colloquial 'you', "
        "missing transition, vague referent) — NEVER vague effects like 'could be clearer/stronger/more "
        "direct'. Note when the SAME phenomenon recurs in multiple sentences. (2) Select the "
...
# Concrete phenomenon micro-lessons per operation domain (spec §2,§8): each names the CONCRETE
# writing phenomenon (what to NOTICE + WHY it affects the reader) and the reflexive-question CATEGORY
# it comes from — so coaching never degrades to "make this clearer/stronger". Deterministic + compact.
_SC_PHENOMENON = {
    "orient_reader": {"category": "naive-reader / setup",
        "notice": "this sentence assumes background a first-time reader may not have yet",
        "why": "a reader who cannot supply the missing setup loses the thread"},
    "complete_meaning": {"category": "naive-reader / elaboration",
        "notice": "the idea is started but stops before a reader has enough to fully understand it",
        "why": "it leaves a reader with the very question the sentence just raised"},
    "make_precise": {"category": "naive-reader / precision",
        "notice": "a word or phrase here can point to more than one meaning",
        "why": "an imprecise word makes a reader guess which meaning you intend"},
    "organize_sentence": {"category": "structure / one job per sentence",
        "notice": "this sentence is carrying more than one complete idea at once (run-on / fused)",
        "why": "when several ideas share one sentence a reader cannot tell which is the main one"},
    "strengthen_expression": {"category": "structure / verbs & register",
        "notice": "the main verb is a form of 'to be' or a weak/informal construction",
        "why": "a linking or weak verb often hides the real action or relationship you mean"},
    "improve_readability": {"category": "structure / syntax",
        "notice": "the word order makes the sentence stumble on a first read (it 'sounds funny')",
        "why": "awkward order forces a reader to reread to recover your meaning"},
    "connect_thought": {"category": "structure / transitions",
        "notice": "the link between this sentence and the one beside it is not stated",
        "why": "without the connection shown a reader cannot see how your ideas relate"},
}


def _sc_concrete_phenomenon(active: Dict[str, Any], domain: str) -> str:
    """Return the ACTUAL observed pattern phrase for the selected domain in this sentence (so coaching
    is grounded in what was really detected), else ''."""
    for x in (active.get("observed_sentence_patterns") or []):
        if sc_domain_for_pattern(x) == domain:
            return str(x).strip()
    return ""
...
def sc_select_operation(active: Dict[str, Any], patterns: List[Dict[str, Any]],
                        para_recurrence: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
    """COORDINATED STRUCTURAL JUDGMENT (deterministic). Coordinates: current writing evidence,
    part-whole relevance, developmental leverage, and recurring learner-pattern evidence — where
    recurrence is measured BOTH across turns (persisted `patterns`) AND across the CURRENT paragraph
    (`para_recurrence`: domain -> #sentences exhibiting it), so a high-leverage pattern that repeats
    in the paragraph is recognised immediately (spec §5,§7). Returns {'decision', 'operation',
    'scaffold_level', 'domain', 'pattern_recurrent', 'recurrence_count', 'reason'}."""
    para_recurrence = para_recurrence or {}
    ...
    if importance in ("distracting", "optional") and priority != "no_instruction_needed":
        return {"decision": "route_upward", "operation": "", "scaffold_level": "",
                "domain": "", "pattern_recurrent": False, "recurrence_count": 0,
    ...
    if not domains and priority in ("no_instruction_needed", "useful_later", ""):
        return {"decision": "advance", "operation": "", "scaffold_level": "",
                "domain": "", "pattern_recurrent": False, "recurrence_count": 0,
                "reason": "sentence is sufficient"}

    def _recurrent(d):
        p = _sc_find_pattern(patterns, d)
        hist = bool(p and int(p.get("instances", 0)) >= 2 and p.get("status") != "controlled")
        return hist or int(para_recurrence.get(d, 0)) >= 2

    # candidate domains present in THIS sentence, scored by (recurring pattern weight) + leverage.
    def _score(d):
        # a recurring, high-leverage pattern (whole-paragraph OR history) gets substantial weight,
        # but leverage still counts so it is never an automatic win (spec §6,§7).
        return (2 if _recurrent(d) else 0) + _SC_LEVERAGE.get(d, 0) / 10.0 + \
               (0.5 * min(int(para_recurrence.get(d, 0)), 4))
    ...
    return {"decision": "operate", "operation": best,
            ...
            "recurrence_count": int(para_recurrence.get(best, 0)),
...
async def _sentence_craft_turn(state, assignment: str, draft: str, dco: Dict[str, Any],
    ...
    # PARAGRAPH-WIDE diagnosis (spec §4,§5): count how many sentences exhibit each phenomenon domain
    # across the WHOLE paragraph (not just the active sentence). This transient recurrence view feeds
    # coordinated judgment so a repeated, high-leverage pattern is recognised immediately.
    para_recurrence: Dict[str, int] = {}
    for s in sents:
        for d in {sc_domain_for_pattern(x) for x in (s.get("observed_sentence_patterns") or [])}:
            if d:
                para_recurrence[d] = para_recurrence.get(d, 0) + 1
    domains = [d for d in (sc_domain_for_pattern(x) for x in (active.get("observed_sentence_patterns") or [])) if d]
    # accumulate provisional CROSS-TURN pattern evidence from THIS sentence (learner history)
    state.sc_patterns = sc_update_patterns(state.sc_patterns or [], domains)

    sel = sc_select_operation(active, state.sc_patterns, para_recurrence)
    _sel_domain = sel.get("domain") or ""
    _concrete = _sc_concrete_phenomenon(active, _sel_domain)
    _ph = _SC_PHENOMENON.get(sel.get("operation") or "", {})
    diagnostics.update({"active_sentence_index": active_i,
                        ...
                        "paragraph_recurrence": para_recurrence,
                        "paragraph_candidate_phenomena": sorted(
                            {x for s in sents for x in (s.get("observed_sentence_patterns") or [])}),
                        "selected_phenomenon": _concrete or _ph.get("notice", ""),
                        "reflexive_category": _ph.get("category", ""),
                        "selection": sel})
    ...
    # Build the learner-facing move. It must ALWAYS name the CONCRETE phenomenon (never "make this
    # clearer/stronger"). Prefer the cognition's guided lesson when it targets THIS sentence (richest,
    # already phenomenon-specific); otherwise compose a concrete lesson from the detected phenomenon +
    # its reflexive category. Explicitness fades with scaffold; recurring patterns generalise (§8,§9).
    sent_text = active.get("exact_sentence_text", "")
    ph = _SC_PHENOMENON.get(op, {})
    notice = _concrete or ph.get("notice", "this sentence could serve the reader better")
    why = ph.get("why", "")
    recurrent = bool(sel.get("pattern_recurrent"))
    rcount = int(sel.get("recurrence_count", 0) or 0)
    # §9 — when a pattern recurs across the paragraph, name it and orient the learner to the pattern.
    pat_note = ""
    if recurrent and rcount >= 2:
        pat_note = (f"I'm noticing something that happens in several of your sentences — {notice}. "
                    f"It shows up about {rcount} times here, so learning to catch it yourself will "
                    f"strengthen a lot of your writing, not just this one.\n\n")

    teach = analysis.get("selected_teaching") or {}
    use_teach = isinstance(teach, dict) and teach.get("sentence_index") == active_i \
        and scaffold in ("more_support", "guided_attention")

    if use_teach:
        noticed = (teach.get("pattern_noticed") or "").strip() or notice.capitalize()
        principle = (teach.get("instructional_principle") or "").strip() or why
        invite = (teach.get("learner_invitation") or "").strip() \
            or "What could you change so a reader gets exactly what you mean? Revise it in the paragraph and resubmit."
    ...
    elif scaffold in ("self_monitoring", "independent_check", "independent"):
        # fade support: hand the NAMED phenomenon to the learner to find (not a generic "make it better")
        invitation = preamble + (
            f'{pat_note}Read this sentence again:\n\n"{sent_text}"\n\n'
            f'Looking at it as a reader who doesn\'t already know what you mean — {notice}. '
            + (f'({why}.) ' if why else "")
            + "Can you spot it and revise it yourself, then tell me what you changed and why?"
        )
    else:
        # concrete, guided move grounded in the detected phenomenon + why it matters to the reader
        invitation = preamble + (
            f'{pat_note}Let\'s look at this sentence:\n\n"{sent_text}"\n\nHere, {notice}'
            + (f" — {why}" if why else "") + ". "
